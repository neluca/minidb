...
export interface CompactionTarget {
  ...
  valueReader?: { close(): void; open(): void; reopenBoth(): void };
...
async function runCompaction(db: CompactionTarget): Promise<void> {
  ...
  try {
    ...
    // On Windows an open read handle blocks renaming the WAL/snapshot files.
    // Close the value reader for the rotation and reopen it after the new files
    // are in place.
